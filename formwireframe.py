...
from models.geometry import warp_lines_torch
from drawing import plot_images, plot_lines
...
class XFeatWireframe(BaseModel):
    default_conf = {
        'sp_params': {
            'has_detector': True,
            'has_descriptor': True,
    ...
    def _init(self, conf):
        self.conf = conf
        xfeat_weight = '/home/wenhuanyao/lcnn/pretrained/xfeat_cityscapes_160k_trainingbase.pth'
        self.xfeat = XFeat(xfeat_weight)
    ...
    def _forward(self, data):
        ...
        pred = self.xfeat.detectForWire(data['image'], top_k=2000)
...
if __name__ == '__main__':
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    img_path0 = '/home/wenhuanyao/lcnn/demo_figs/berlin_000003_000019_leftImg8bit.png'
    gray0 = cv2.imread(img_path0, 0)
    torch_gray0 = numpy_image_to_torch(gray0)
    torch_gray0 = torch_gray0.to(device)[None]
    data = {'image': torch_gray0}
    
    xw_model = XFeatWireframe(conf)
    pred = xw_model(data)
    lines = pred['lines'][0]
    ...
    plot_images([img0, img0], ['Image 1 - detected lines', 'Image 1 - detected lines'], pad=0.5)
    plot_lines([lines, lines], ps=3, lw=2)
    plt.savefig('savefig_line_example.png')
    pass